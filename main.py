...
class PupilCameraViewer(QWidget):
    ...
    def toggle_pupil_detection(self):
        ...
        if self.pupil_detection_mode:
            ...
            self.pupil_align_button.setEnabled(True)  # 启用对齐按钮
        else:
            self.pupil_detect_button.setText("开始瞳孔检测")
            self.pupil_detect_button.setStyleSheet("QPushButton { background-color: #4CAF50; color: white; }")
            self.sharpness_text.setText("瞳孔检测模式已关闭")
            self.pupil_info_text.clear()
            self.pupil_align_button.setEnabled(False)  # 禁用对齐按钮
            # 如果正在对齐，也要停止
    ...
    def perform_alignment(self, pupil_x, pupil_y):
        """执行瞳孔对齐控制"""
        ...
        if abs(error_y) < self.alignment_tolerance:
            self.y_aligned = True
        else:
            self.y_aligned = False
        ...
        if not self.x_aligned:
            # 获取当前电机位置
            # current_x_pos = self.motor_controller.get_x_position()

            # PID计算
            control_x = self.pid_x.update(error_x)

            # 转换为毫米单位的移动量
            move_x_mm = int(control_x * self.pixel_to_mm_ratio) * 500

            # 计算新的绝对位置
            # new_x_pos = current_x_pos + move_x_mm

            # 移动电机
            # self.motor_controller.move_x_to_absolute(new_x_pos)

            # t1 = threading.Thread(target=self.motor_controller.move_x_to_relative(move_x_mm), name='Motorx_Thread')
            # t1.start()
            # t1.join()
            self.motor_controller.move_x_to_relative(move_x_mm)

        # Y轴控制
        if not self.y_aligned:
            # 获取当前电机位置
            # current_y_pos = self.motor_controller.get_y_position()

            # PID计算
            control_y = self.pid_y.update(error_y)

            # 转换为毫米单位的移动量
            move_y_mm = int(control_y * self.pixel_to_mm_ratio) * 500

            # 计算新的绝对位置
            # new_y_pos = current_y_pos - move_y_mm
    ...
    def process_pupil_detection(self, img):
        ...
        if pupil_circle is not None:
            ...
            # 绘制连接线（黄色）
            # cv2.line(display_img, (x, y), (target_x, target_y), (0, 255, 255), 1)
