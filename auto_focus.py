...
class AutoFocusStateMachine(QObject):
    ...
    def __init__(self, motor_controller, detect_func, sharpness_func):
        ...
        self.config = {
           ...
           'fine_focus_scan_range': 12.0,  # 连续扫描总范围 (mm)
            'fine_focus_sample_period': 0.03, # 采样周期 (s)，比如 50ms
            'fine_focus_settle_time': 0.05  # 起始/停止时的静置时间 (s)
    ...
    def _run_focus_sequence(self):
        ...
        try:
            ...
            # final_y, final_sharpness = self._fine_focus_continuous_pp_quartic_func(pupil_position)
    ...
    def _fine_focus_continuous_pp(self, start_y: float) -> Tuple[Optional[float], Optional[float]]:
        ...
        sample_period = float(self.config.get('fine_focus_sample_period', 0.04))  # 采样周期 s
    ...
    def _fine_focus_continuous_pp_quartic_func(self, start_y: float) -> Tuple[Optional[float], Optional[float]]:
        """
        精细对焦（连续扫描版，位置模式 + 四次函数拟合）：
        从 start_y 开始往正方向连续走一段距离（默认 12mm），
        在运动过程中不断采样 (y, sharpness)，
        扫描结束后对采样点做四次函数拟合，取拟合曲线峰值位置为最佳焦点。
        """
        self.message_updated.emit(
            f"[Continuous-PP] Starting fine focus from {start_y:.3f}mm"
        )
        ...
        if len(sampled_data) < 3:
            ...
            # 四次多项式模型：y = a4*x^4 + a3*x^3 + a2*x^2 + a1*x + a0
            def quartic(x, a4, a3, a2, a1, a0):
            ...
            try:
                # np.polyfit 返回 [a4, a3, a2, a1, a0]
                coeffs = np.polyfit(x_data, y_data, 4)
                a4, a3, a2, a1, a0 = [float(c) for c in coeffs]
                popt = (a4, a3, a2, a1, a0)

                min_pos, max_pos = float(np.min(x_data)), float(np.max(x_data))

                # 求导：y' = 4a4*x^3 + 3a3*x^2 + 2a2*x + a1
                d_coeffs = np.array([4 * a4, 3 * a3, 2 * a2, a1], dtype=float)
                roots = np.roots(d_coeffs)  # 可能有复根

                # 候选点：区间内的实根 + 两端点
                candidates = [min_pos, max_pos]
                for r in roots:
                    if np.isfinite(r):
                        if abs(r.imag) < 1e-8:
                            xr = float(r.real)
                            if min_pos <= xr <= max_pos:
                                candidates.append(xr)

                # 评估并选最大值
                cand_vals = [quartic(xc, *popt) for xc in candidates]
                idx = int(np.argmax(cand_vals))
                best_y = float(candidates[idx])
                best_sharpness = float(cand_vals[idx])

                reasons = []
                if not np.isfinite(best_y) or not np.isfinite(best_sharpness):
                    reasons.append("non-finite result")
                if best_sharpness <= 0:
                    reasons.append("peak <= 0")

                if not reasons:
                    self.message_updated.emit(
                        f"[Continuous-PP] Quartic fit OK: peak_y={best_y:.3f}, peak_sharpness≈{best_sharpness:.2f}"
                    )
                else:
                    self.message_updated.emit(
                        f"[Continuous-PP] Quartic fit unreliable: {', '.join(reasons)}, "
                        "fallback to max-sharpness sample."
                    )
                    best_sample = max(sampled_data, key=lambda d: d['sharpness'])
                    best_y = best_sample['position']
                ...
                # 拟合失败，退回最大样本点
                self.message_updated.emit(
                    f"[Continuous-PP] Quartic fitting failed: {e}, fallback to max-sharpness sample."
                )
                best_sample = max(sampled_data, key=lambda d: d['sharpness'])
                best_y = best_sample['position']
                best_sharpness = best_sample['sharpness']
